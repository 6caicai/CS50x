...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    compare = strategy(99, 99)
    for i in range(99):
        for j in range(99):
            if strategy(i, j) != compare:
                return False
    return True
    # END PROBLEM 7
...
def make_averaged(original_function, total_samples=1000):
    ...
    def averaged(*args):
        ...
        for i in range(total_samples):
            accumulate = accumulate + original_function(*args)
        average = accumulate / total_samples
        return average
    return averaged
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    rolls = 1
    average = 0
    function = make_averaged(roll_dice, total_samples)
    for i in range(10):
        tmp = function(i + 1, dice)
        if tmp > average:
            rolls = i + 1
            average = tmp
    return rolls
