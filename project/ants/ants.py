...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            assert place.ant.can_contain(self) or self.can_contain(place.ant) is True, 'Two ants in {0}'.format(place)
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt eat ramdom bee in its place and chewing for three turns."""
    ...
    def action(self, gamestate):
        """eat a bee if there is a bee in its place and turns_to_chew=0, do nothing if there isn't any bee in its place, and reduce turns_to_chew if it isn't zero."""
...
class TankAnt(ContainerAnt):
    """a TankAnt is a container ant which can protect the ant it contained and deals 1 damage to the bees in its place each turn."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaTrower is a throwerant that is waterproof"""
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        super().action(gamestate)
        if self.current_place.exit != None:
            current_place = self.current_place.exit
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        dict = {}
        current_place = self.place
        while current_place.entrance.is_hive != True:
            if current_place.ant != None and current_place.ant is not self:
                dict[current_place.ant] = distance
                if current_place.ant.is_container == True and current_place.ant.ant_contained != None:
                    dict[current_place.ant.ant_contained] = distance
            if current_place.bees != None:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return max(0, self.base_demage - 0.0625 * self.insects_shot - 0.25 * distance)
        # END Problem Optional 2
