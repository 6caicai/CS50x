...
def balanced(m):
    ...
    if is_planet(m):
        ...
        return length(left(m)) * mass(end(left(m))) - length(right(m)) * mass(end(right(m)))  == 0

    else:
        return balanced(end(right(m))) and balanced(end(left(m))) and length(left(m)) * total_weight(end(left(m))) - length(right(m)) * total_weight(end(right(m))) == 0
...
def has_path(t, word):
    ...
    if len(word) == 1:
        ...
        list = [has_path(b, word[1:]) for b in branches(t)]
        return list.count(True) != 0
    else:
        return False
